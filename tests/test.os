...
Процедура ТестДолжен_ПроверитьЗамерВремени() Экспорт
	...
	Замер.НачатьЗамер(Дата(2018, 01, 01, 12, 0, 0));
	Замер.ЗакончитьЗамер(Дата(2018, 01, 01, 12, 0, 59));
	
	Ожидаем.Что(Замер.ПолучитьРезультат()).Равно(59);
	
	Замер.НачатьЗамер(Дата(2018, 01, 01, 12, 0, 0));
	Замер.ЗакончитьЗамер(Дата(2018, 01, 01, 12, 1, 0));
	
	Ожидаем.Что(Замер.ПолучитьРезультат()).Равно(60);
	
	Замер.НачатьЗамер(Дата(2018, 01, 01, 12, 0, 1));
	Замер.ЗакончитьЗамер(Дата(2018, 01, 02, 12, 0, 2));
	
	Ожидаем.Что(Замер.ПолучитьРезультат()).Равно(86401);
...
Процедура ТестДолжен_ПроверитьФорматированиеЗамераВремени() Экспорт
	...
	Ожидаем
		.Что(Замер.ФорматироватьРезультат(59)).Равно("59с.")
		.Что(Замер.ФорматироватьРезультат(Минута * 2 + 3)).Равно("2м., 3с.")
		.Что(Замер.ФорматироватьРезультат(Час * 2 + Минута * 4 + 15)).Равно("2ч., 4м., 15с.")
		.Что(Замер.ФорматироватьРезультат(День * 2 + Час * 13 + 45)).Равно("2д., 13ч., 0м., 45с.");
